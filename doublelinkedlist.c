...
NODE *p, *newnode; if(countnodes(head)==MAX) printf("\nList is Full!!"); 
else
{
if(head==NULL)
{
...
NODE *p;
 if(head==NULL)
printf("\nList is Empty (QUEUE)"); else
{
p=head;
head=head->right; head->right->left=NULL;
free(p);
...
printf("\n1.QUEUE- Insert at Rear & Delete from Front"); printf("\n2.QUEUE- Insert at Front & Delete from Rear"); printf("\n3.Exit");
printf("\nEnter your choice: ");
scanf("%d", &ch);
switch(ch)
{
case 1: do{
printf("\n1.Insert at Rear\t2.Delete from Front\t3.Exit"); printf("\nEnter your choice: ");
scanf("%d", &ch1);
switch(ch1)
{
...
while(ch!=3); head=display(head); return head;
}
...
switch(ch)
{
case 1: printf("\nHow many employees data you want to create: "); scanf("%d", &n);
for(i=0;i<n;i++)
